        ...
        while curr_steps < TrainingParameters.N_MAX_STEPS:
            ...
            if demon:
                ...
                for results in range(done_len):
                    ...
                    mb_c_i.append(job_results[results][14])
                    curr_episodes += job_results[results][-2]
                ...
                if RecordingParameters.TENSORBOARD:
                    ...
                    global_summary.add_scalar("Train/Comm_Loss", mb_loss[-1][-1], curr_steps)
            ...
            if (curr_steps - last_test_t) / RecordingParameters.EVAL_INTERVAL >= 1.0:
                ...
                if RecordingParameters.TENSORBOARD:
                    ...
                    global_summary.add_scalar("Eval/Comm_Ratio", eval_performance_dict['comm_ratio'], curr_steps)
                    global_summary.add_scalar("Eval/Comm_Per_Agent", eval_performance_dict['comm_per_agent'], curr_steps)

                print('episodes: {}, step: {}, episode reward: {}, final goals: {}, comm ratio: {:.2f}%\n'.format(
                    curr_episodes, curr_steps, eval_performance_dict['per_r'],
                    eval_performance_dict['per_final_goals'], eval_performance_dict['comm_ratio'] * 100))
...
def evaluate(eval_env, episodic_buffer, model, device, save_gif, curr_steps, greedy):
    """Evaluate Model with Communication Telemetry."""
    eval_performance_dict = {'per_r': [], 'per_ex_r': [], 'per_in_r': [], 'per_valid_rate': [], 'per_episode_len': [],
                             ...
                             'comm_ratio': [], 'comm_per_agent': [], 'comm_efficiency': []}
    episode_frames = []
    ...
    for i in range(RecordingParameters.EVAL_EPISODES):
        ...
        while not done:
            actions, pre_block, hidden_state, num_invalid, v_all, ps, raw_message, c_i = model.evaluate(
                obs, vector, valid_actions, hidden_state, greedy, episodic_buffer.no_reward, message, num_agent)
            
            # Cache update in evaluation
            message = c_i * raw_message + (1.0 - c_i) * message
            total_transmissions += c_i.sum().item()
            ...
            if done:
                ...
                comm_ratio = total_transmissions / total_opportunities
                comm_per_agent = total_transmissions / num_agent
                comm_efficiency = one_episode_perf['episode_reward'] / max(1.0, total_transmissions)

                eval_performance_dict['comm_ratio'].append(comm_ratio)
                eval_performance_dict['comm_per_agent'].append(comm_per_agent)
                eval_performance_dict['comm_efficiency'].append(comm_efficiency)
                ...
                if save_gif:
                    ...
                    make_gif(images,
                             '{}/steps_{:d}_reward{:.1f}_goals{:.1f}_comm{:.2f}.gif'.format(
                                 RecordingParameters.GIFS_PATH,
                                 curr_steps, one_episode_perf['episode_reward'],
                                 num_on_goals, comm_ratio))
                    save_gif = False
